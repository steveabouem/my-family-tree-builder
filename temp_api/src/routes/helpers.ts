...
export const sendRouteHandlerResponse = <RequestPayload, ResponseType>(
  ...
  // @ts-ignore
  logger.info('Use helper ', { isUserAuthenticated: isUserAuthenticated(request), request })
  action(requestPayload)
    .then((data: ServiceResponseWithPayload<ResponseType>) => {
      logger.info('Call successful', { actionName, data });
      if (request &&  request?.session && request?.sessionID && data.payload && data?.addToSession) {
        //! do not spread or destructure session object, it will remove all built in features and crash the server
        request.session.details = data.payload;
        // @ts-ignore
...
export const isUserAuthenticated = (req: Request): boolean => {
  ...
  logger.info('SESSION EXISTS ', {s: req?.session})
  if (!req?.session) {
